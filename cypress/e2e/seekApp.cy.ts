describe("Home Page", () => {
  it("checks the main title, description, and register button functionality", () => {
    ...
    cy.get("#cypress-title-home").contains("con Seek");

    // Verificar la descripción principal
    cy.get("#cypress-description-home").contains(
      "¡Descubre cómo organizar y supervisar tus pendientes nunca fue tan fácil! Con Seek, puedes visualizar tus tareas, actualizarlas en diferentes estados y mantener todo bajo control. Una app diseñada para ayudarte a priorizar, completar tus objetivos y simplificar tu vida, sin complicaciones."
    );
...
describe("Footer Component", () => {
  ...
  it("checks the main heading and text in the footer", () => {
    ...
    cy.contains(
      "Con Seek, cada tarea importa. Organiza, supervisa y alcanza tus metas con facilidad."
    ).should("be.visible");

    // Verificar el texto secundario
    cy.contains(
      "Haz que cada tarea cuente con Seek y alcanza tus metas fácilmente."
    ).should("be.visible");
  });
